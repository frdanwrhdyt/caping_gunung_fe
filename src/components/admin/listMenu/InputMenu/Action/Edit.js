...
export default function Edit({ data, content }) {
  const [isOpen, setIsOpen] = useState(false);
  const [isData, setIsData] = useState([]);
  const [isKeys, setIsKeys] = useState([]);

  const devisi = ["Coffee Corner", "Dapur"];

  function openModal() {
    setIsData(data);
    setIsKeys(Object.keys(data));
    setIsOpen(true);
  }
  function closeModal() {
    setIsOpen(false);
                ...
                >
                  ...
                  <div className="flex flex-col justify-around space-y-5 h-full ">
                    <div>
                      ...
                      {content ? (
                        <Dropdown content={devisi} title={isData["Devisi"]} />
                      ) : null}
                    </div>
